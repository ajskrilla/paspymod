...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Add a list of accounts from a CSV file, JSON file, or list. All headers in file match the values of the API page. Check example_files to compare.")
    ...
    parser.add_argument('-l','--List', type=ast.literal_eval, required=False, default=None, help= 'Array of accounts. Please input as a list such as .')
    parser.add_argument('-j','--JSON', type=str, required=False, default=None, help= 'JSON file of accounts. Please input the full file path.')
    args = parser.parse_args()

log.info("Using endpoint /ServerManage/AddAccounts...")
...
if args.Path != None:
    ...
    with open(path, 'r') as f:
        ...
        other_requests(Call='/ServerManage/AddAccounts', Accounts=data, Debug=True)

elif args.List != None:
    log.info("List of accounts is: {0}".format(args.List))
    other_requests(Call='/ServerManage/AddAccounts', Accounts=args.List, Debug=True)
    ...
    with open(path, 'r') as f:
        other_requests(Call='/ServerManage/AddAccounts', Accounts=json.load(f), Debug=True)
